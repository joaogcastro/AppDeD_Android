...
class MainActivity : AppCompatActivity() {
        ...
        createPlayerButton.setOnClickListener {
            ...
            lifecycleScope.launch {
                try{
                    ...
                    loadPlayers()
                    Toast.makeText(this@MainActivity, "Player salvo com sucesso!", Toast.LENGTH_SHORT).show()
                } catch (e: Exception) {
                    Toast.makeText(this@MainActivity, "Erro ao salvar o player: $e", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun displayPlayers(players: List<Player>) {
        ...
        for (player in players) {
            stringBuilder.append("Nome: ${player.name}, Raça: ${player.race?.name}\n")
        }
        playersTextView.text = stringBuilder.toString()
    }
